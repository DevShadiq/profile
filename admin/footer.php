    ...
    <div class="sidenav-backdrop backdrop"></div>
            ...
            <footer class="page-footer">
                <div class="font-13">2021 © <b>NACTAR</b> - All rights reserved.</div>
                <a class="px-4" href="https://www.facebook.com/ronyislamweb/" target="_blank">Rony Islam</a>
...
<script type="text/javascript">
 $('.select2').select2();
        ...
        $("#addRow").click(function () {
            ...
            html += '<select class="form-control select2" name="pro_name[]"><option value="">Product</option><?php $pro = "select * from product order by pro_id asc";$pro_result = $conn->query($pro);while($pro_row = $pro_result->fetch_assoc()){$p_id = $pro_row['pro_id'];$p_sql = "SELECT SUM(stc_pro_quantity) as p_qnt FROM stockin where stc_pro_name = '$p_id'";$p_result = $conn->query($p_sql);$p_row = $p_result->fetch_assoc();$p_count = $p_row['p_qnt'];$p_sql2 = "SELECT SUM(sto_pro_qnt) as p_qnt2 FROM stockout where sto_pro_id = '$p_id'";$p_result2 = $conn->query($p_sql2);$p_row2 = $p_result2->fetch_assoc();$p_count2 = $p_row2['p_qnt2'];$count = $p_count-$p_count2;?><option value="<?php echo $pro_row['pro_id'];?>"><?php echo $pro_row['pro_name'];?><?php echo '&nbsp;('.$count.')';?></option><?php } ?> </select>';
            html += '</div>';
			html += '<div class="form-group stock-input">';
            html += '<select class="form-control select2" name="sup_name[]"><option value="">Supplier</option><?php $sup = "select * from supplier where sup_status = 'active' order by sup_id asc";$sup_result = $conn->query($sup);while($sup_row = $sup_result->fetch_assoc()){ ?><option value="<?php echo $sup_row['sup_id'];?>"><?php echo $sup_row['sup_name'];?></option><?php } ?></select>';
            html += '</div>';
			html += '<div class="form-group stock-input">';
            html += '<select class="form-control select2" name="bra_name[]"><option value="">Brand</option><?php $bra = "select * from brand where bra_status = 'active' order by bra_id asc";$bra_result = $conn->query($bra);while($bra_row = $bra_result->fetch_assoc()){ ?><option value="<?php echo $bra_row['bra_name'];?>"><?php echo $bra_row['bra_name'];?></option><?php } ?></select>';
            html += '</div>';
			html += '<div class="form-group stock-input">';
            ...
            html += '</div>';
			html += '<div class="form-group stock-input">';
            html += '<select class="form-control" name="course_name[]"><option value="">Course</option><?php $cou = "select * from course order by cou_id asc";$cou_result = $conn->query($cou);while($cou_row = $cou_result->fetch_assoc()){ ?><option value="<?php echo $cou_row['cou_name'];?>"><?php echo $cou_row['cou_name'];?></option><?php } ?></select>';
            html += '</div>';
			html += '<div class="form-group stock-input">';
